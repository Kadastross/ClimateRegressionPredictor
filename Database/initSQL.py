...
cursor = db.cursor()
cursor.execute("USE sql9379233;")
cursor.execute("CREATE TABLE User (UserID VARCHAR(30) PRIMARY KEY, Password VARCHAR(30) NOT NULL)")
cursor.execute("CREATE TABLE Simulation (SimulationID INT NOT NULL AUTO_INCREMENT, SimulationName INT NOT NULL, UserID VARCHAR(30) NOT NULL, KEY `id` (SimulationID), FOREIGN KEY (UserID) REFERENCES User(UserID) ON UPDATE CASCADE ON DELETE CASCADE, PRIMARY KEY(UserID, SimulationName))")
